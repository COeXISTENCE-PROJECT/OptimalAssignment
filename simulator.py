...
class SumoSimulator():
    ...
    def save_snapshot(self, episode_num, snapshot_idx):
        """Zapisuje zrzut stanu symulacji do pliku XML."""
        snapshot_path = os.path.join(
        self.sumo_save_path, 
        f"snapshot_ep{episode_num}_{snapshot_idx}.xml"
        )
        try:
            self.sumo_connection.simulation.saveState(snapshot_path)
        except Exception as e:
            print(f"Błąd zapisu snapshotu: {e}")

    def save_snapshot_two(self, episode_num, snapshot_idx):
        snapshot_data = []
        ...
        for v_id in vehicle_ids:
            snapshot_data.append({
                'agent_id': v_id,
                'edge_id': self.sumo_connection.vehicle.getRoadID(v_id),
                'lane_index': self.sumo_connection.vehicle.getLaneIndex(v_id),
                'position': round(self.sumo_connection.vehicle.getLanePosition(v_id), 2),
                'speed': round(self.sumo_connection.vehicle.getSpeed(v_id), 2),
                'type': self.sumo_connection.vehicle.getTypeID(v_id)
            })
        df = pd.DataFrame(snapshot_data)
        path = os.path.join(self.sumo_save_path, f"es_ep{episode_num}_{snapshot_idx}.csv")
