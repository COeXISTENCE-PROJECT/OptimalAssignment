...
for series in range(100):  # Assuming 10 series of experiments
    ...
    for (exp_id, time), group in grouped:
        ...
        matrix = np.zeros((num_edges, num_edges), dtype=int)

        for path in group["path"]:
            for i in range(len(path) - 1):
                try:
                    u = edge_to_idx[path[i]]
                    v = edge_to_idx[path[i + 1]]
                    matrix[u, v] = 1
    ...
    np.savez_compressed(f"adjacency_matrices{series}.npz", **save_dict)
    print(f"Saved {len(save_dict)} matrices to adjacency_matrices.npz")
