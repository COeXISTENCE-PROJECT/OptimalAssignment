...
for batch_idx in range(100):
    ...
    for (exp_id, time), group in grouped:
        ...
        matrix = np.zeros((num_hexes, num_hexes), dtype=np.int8)  # int8 saves space

        for h_path in group["hex_path"]:
            # Use 'j' here to avoid conflict with 'batch_idx' or 'i'
            for j in range(len(h_path) - 1):
                u_hex, v_hex = h_path[j], h_path[j + 1]

                if u_hex in hex_to_idx and v_hex in hex_to_idx:
                    u_idx = hex_to_idx[u_hex]
                    v_idx = hex_to_idx[v_hex]
    ...
    # Save Batch
    if hex_results:
        fname = f"hex_adjacency_matrices_{start_exp}_{end_exp}.npz"
