...
def run_wavenet_training():
    ...
    """
    # 1. Określenie katalogu bazowego (tam gdzie znajduje się ten skrypt)
    base_dir = os.path.abspath(os.path.dirname(__file__))

    # 2. Identyfikacja eksperymentu (SLURM_JOB_ID jeśli dostępne, inaczej timestamp)
    job_id = os.environ.get("SLURM_JOB_ID", datetime.now().strftime("%Y%m%d_%H%M%S"))

    # 3. Definicja ścieżek bezwzględnych
    data_dir = os.path.join(base_dir, "data", "WAVENET_READY")
    adj_path = os.path.join(base_dir, "data", "adjacency_matrix.csv")

    # Utworzenie unikalnego katalogu dla tego konkretnego zadania
    save_dir = os.path.join(base_dir, "garage", f"experiment_{job_id}")
    os.makedirs(save_dir, exist_ok=True)

    # Parametr --save w train.py jest używany jako prefix zapisu (np. save_dir/model_epoch_1.pth)
    ...
    command = [
        ...
        "--device", "cuda:0",  # Zmieniono na cuda:0 zakladajac dostep do GPU na slurm
        "--data", data_dir,
        "--adjdata", adj_path,
...
def main():
    parser = argparse.ArgumentParser(description="Analiza statystyk treningowych Graph WaveNet")
    parser.add_argument('csv_path', type=str, help='Ścieżka do pliku training_metrics.csv')
    ...
    # Wczytanie danych z wykorzystaniem Pandas
    try:
    ...
    # Pozwala to ocenić, jak bardzo model jest dopasowany do zbioru treningowego względem walidacyjnego
    ...
    print(f"  • Minimum Loss (MAE): {best_val_loss:.4f} (Osiągnięte w epoce: {df.loc[best_loss_idx, 'epoch']})")
    print(
    ...
    if gen_gap > (0.2 * corresponding_train_loss):
        print("  ! UWAGA: Znaczna różnica między błędem treningowym a walidacyjnym (powyżej 20%).")
        print("    Może to sugerować wczesne stadium przeuczenia modelu (overfitting).")

    # 4. Podgląd ostatnich 5 epok (trend)
